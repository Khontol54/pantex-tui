...
"""Tests for pantex_sdk — signers and SDK."""
...
from pantex_sdk._sdk import _b64_encode
...
BASE_URL = "https://api.test.pantex.io"
...
@pytest.fixture
def sdk(operator, intent) -> PantexSDK:
    return PantexSDK(
        operator,
        intent,
        base_url=BASE_URL,
...
class TestPantexSDKInit:
    def test_base_url_trailing_slash_stripped(self, operator):
        sdk = PantexSDK(operator, base_url="https://example.com/", api_key_path=None)
...
class TestPantexSDKApiKeyPersistence:
    def test_load_and_save(self, operator, tmp_path):
        key_path = str(tmp_path / "api_key.txt")
        sdk = PantexSDK(operator, base_url=BASE_URL, api_key_path=key_path)
        assert sdk._api_key is None

        sdk._api_key = "my-secret-key"
        sdk._save_api_key()

        sdk2 = PantexSDK(operator, base_url=BASE_URL, api_key_path=key_path)
    ...
    def test_save_bare_filename(self, operator, tmp_path, monkeypatch):
        ...
        sdk = PantexSDK(operator, base_url=BASE_URL, api_key_path="api_key.txt")
        sdk._api_key = "secret"
        sdk._save_api_key()
...
class TestRequireKey:
    def test_present(self):
        assert PantexSDK._require_key({"a": 1}, "a") == 1

    def test_missing(self):
        with pytest.raises(PantexError, match="Missing required key 'x'"):
            PantexSDK._require_key({"a": 1}, "x")

    def test_context_in_message(self):
        with pytest.raises(PantexError, match="auth challenge"):
            PantexSDK._require_key({}, "message", " (auth challenge)")
...
class TestRequest:
    ...
    async def test_401_raises_auth_error(self, authed_sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexAuthError) as exc_info:
    ...
    async def test_400_raises_api_error(self, authed_sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexAPIError) as exc_info:
    ...
    async def test_invalid_json_raises(self, authed_sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexError, match="Invalid JSON"):
                await authed_sdk._request("GET", "/v1/test")
        await authed_sdk.close()
    ...
    async def test_network_error_exhausted(self, authed_sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexError, match="failed after"):
                await authed_sdk._request("GET", "/v1/test")
        await authed_sdk.close()
...
class TestAuthenticate:
    ...
    async def test_missing_challenge_key(self, sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexError, match="message"):
...
class TestBuildSignSubmit:
    ...
    async def test_intent_without_signer(self, operator):
        sdk = PantexSDK(operator, None, base_url=BASE_URL, api_key_path=None)
        sdk._api_key = "key"
    ...
    async def test_missing_context_key(self, authed_sdk):
        with aioresponses() as m:
            ...
            with pytest.raises(PantexError, match="context"):
...
class TestPublicAPIMethods:
    async def test_get_account_info(self, authed_sdk):
        with aioresponses() as m:
            m.get(
                ...
                payload={
                    "party_id": {"address": "Pantex::1220xyz"},
                    "user_id": "uid-info",
                    "tokens": [
                        {
                            "instrument_id": "USDC",
            ...
            assert result.address == "Pantex::1220xyz"
    ...
    async def test_get_account_admin(self, authed_sdk):
        with aioresponses() as m:
            ...
            assert result.address == "Pantex::1220abc"
...
class TestSessionLifecycle:
    async def test_context_manager(self, operator):
        async with PantexSDK(operator, base_url=BASE_URL, api_key_path=None) as sdk:
...
class TestAccountAdmin:
    def test_from_raw_full(self):
        ...
        assert admin.address == "Pantex::1220abc"

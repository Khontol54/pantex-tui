...
class WalletConfig:
    ...
    def load(self):
        ...
        self.operator_key      = raw.get("PANTEX_OPERATOR_KEY", "")
        self.trading_key       = raw.get("PANTEX_TRADING_KEY", "")
        self.base_url          = raw.get("PANTEX_BASE_URL", "https://api.pantex.io")
    ...
    def save(self):
        lines = [
            f"PANTEX_OPERATOR_KEY={self.operator_key}",
            f"PANTEX_TRADING_KEY={self.trading_key}",
            f"PANTEX_BASE_URL={self.base_url}",
...
class WalletBot:
    ...
    async def _run(self):
        ...
        if not self.cfg.operator_key or not self.cfg.trading_key:
            self.state.log("Missing PANTEX_OPERATOR_KEY or PANTEX_TRADING_KEY", "error")
        ...
        try:
            ...
            async with PantexSDK(operator, intent, base_url=self.cfg.base_url) as sdk:
    ...
    async def _execute_swap(self, sdk: PantexSDK):
        cfg   = self.cfg
        state = self.state

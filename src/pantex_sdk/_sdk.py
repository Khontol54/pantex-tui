...
class PantexError(Exception):
    """Base exception for all Pantex SDK errors."""


class PantexAPIError(PantexError):
...
class PantexAuthError(PantexAPIError):
    """Raised on authentication failures (401/403)."""


class PantexTimeoutError(PantexError):
...
class BaseSigner(ABC):
    """Common interface and key-loading infrastructure for Pantex signers.
...
class PantexSDK:
    ...
    Example::
        ...
        async with PantexSDK(operator, intent) as sdk:
    ...
    def __init__(
        ...
        *,
        base_url: str = "https://api.testnet.pantex.io",
    ...
    def __repr__(self) -> str:
        return (
            f"PantexSDK(base_url={self.base_url!r}, "
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            ...
            self._session = aiohttp.ClientSession(
                ...
                connector=connector,
                headers={"User-Agent": "PantexSDK/1.0"},
            )
    ...
    async def __aenter__(self) -> PantexSDK:
        return self
            ...
            try:
                ...
                async with session.request(
                    ...
                    if resp.status in (401, 403):
                        raise PantexAuthError(resp.status, body)
                    ...
                    if resp.status >= 400:
                        if (
                            ...
                            continue
                        raise PantexAPIError(resp.status, body)
    ...
    async def authenticate(self, *, force: bool = False) -> str:
        ...
        async with self._auth_lock:
            ...
            logger.info("Authenticating with Pantex API...")
        ...
        *,
        ...
        if intent and self._intent_signer is None:
            raise RuntimeError(
                ...
                "Pass one to PantexSDK() to use intent operations."
            )

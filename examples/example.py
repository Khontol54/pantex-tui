...
"""
Pantex SDK -- Example Usage

Set the following environment variables before running:

    export PANTEX_BASE_URL="https://api.testnet.pantex.io"
    export PANTEX_OPERATOR_KEY="<operator Ed25519 private key hex>"
    export PANTEX_TRADING_KEY="<intent secp256k1 private key hex>"
...
async def main() -> None:
    ...
    base_url = os.environ.get("PANTEX_BASE_URL", "https://api.testnet.pantex.io")

    operator_hex = os.environ.get("PANTEX_OPERATOR_KEY")
    if not operator_hex:
        sys.exit("PANTEX_OPERATOR_KEY environment variable is required")

    intent_hex = os.environ.get("PANTEX_TRADING_KEY")
    if not intent_hex:
        sys.exit("PANTEX_TRADING_KEY environment variable is required")
    ...
    async with PantexSDK(operator, intent, base_url=base_url) as sdk:
        ...
        except PantexAuthError as exc:
            log.error("Auth error (HTTP %d): %s", exc.status, exc.body[:100])
        except PantexAPIError as exc:

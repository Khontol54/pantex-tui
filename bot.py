"""
Pantex Trading Bot  —  Rich Terminal UI
...
from pantex_sdk._sdk import InstrumentId
...
STATE_FILE          = Path(os.environ.get("STATE_FILE", "/root/pantex-bot/state.json"))
...
BASE_URL   = os.environ.get("PANTEX_BASE_URL", "https://api.pantex.io")

CC_INSTRUMENT   = InstrumentId(admin=CC_ADMIN,   id=CC_ID)
USDC_INSTRUMENT = InstrumentId(admin=USDC_ADMIN, id=USDC_ID)

logging.getLogger("pantex_sdk").setLevel(logging.WARNING)
console = Console()
...
def render_ui(state: BotState) -> Layout:
    ...
    header_text.append("⬡ PANTEX TRADING BOT ", style="bold bright_cyan")
...
async def execute_swap(sdk: PantexSDK, state: BotState) -> bool:
...
async def run_bot():
    operator_key = os.environ.get("PANTEX_OPERATOR_KEY")
    trading_key  = os.environ.get("PANTEX_TRADING_KEY")
    if not operator_key:
        raise EnvironmentError("PANTEX_OPERATOR_KEY not set")
    if not trading_key:
        raise EnvironmentError("PANTEX_TRADING_KEY not set")
    ...
    async with PantexSDK(operator, intent, base_url=BASE_URL) as sdk:
